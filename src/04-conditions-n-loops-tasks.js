...
function doRectanglesOverlap(rect1, rect2) {
  const noOverlapHorizontally = rect1.left + rect1.width <= rect2.left
    || rect2.left + rect2.width <= rect1.left;
  const noOverlapVertically = rect1.top + rect1.height <= rect2.top
...
function findFirstSingleChar(str) {
  const charCount = {};
  str.split('').forEach((char) => { charCount[char] = (charCount[char] || 0) + 1; });
...
function isCreditCardNumber(ccn) {
  const ccnStr = ccn.toString().split('').reverse().map(Number);
  const totalSum = ccnStr.reduce((sum, digit, index) => {
